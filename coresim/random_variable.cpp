...
/* Static Variable */
StaticVariable::StaticVariable(double value) {
  val_ = value;
}

double StaticVariable::value() {
  return val_;
}
...
int EmpiricalRandomVariable::loadCDF(std::string filename) {
  ...
  this->mean_flow_size = w_sum * (double)params.mss;
...
int EmpiricalBytesRandomVariable::loadCDF(std::string filename) {
  ...
  while (std::getline(myfile, line)) {
    ...
    double num_pkts = std::ceil(flow_sz / params.mss);
...
double EmpiricalBytesRandomVariable::value() {
...
CDFRandomVariable::CDFRandomVariable(std::string filename)
    : EmpiricalRandomVariable(std::move(filename), false) {}

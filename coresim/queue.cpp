...
Queue::Queue(uint32_t id, double rate, uint32_t limit_bytes, int location) {
    ...
    //this->rate = 100*1e9 * ((double)12/13); // in bps
...
// Determine which is the next packet you get if you call deque()
// but don't really deque it from the queue
// Used for multi-threaded version to be able to assign the QueueProcessingEvent
// to the correct EventQueue.
// Since we stick to WFQueue, i don't check the correctness of the following
// impl in this base class.
Packet *Queue::try_deque() {
    if (bytes_in_queue > 0) {
...
Packet *Queue::deque(double deque_time) {
    if (bytes_in_queue > 0) {
...
void ProbDropQueue::enque(Packet *packet) {
    ...
    if (bytes_in_queue + packet->size <= limit_bytes) {
        ...
        bytes_in_queue += packet->size;
        if (!busy) {
            add_to_event_queue(new QueueProcessingEvent(get_current_time(), this, packet->flow->qid));
            //add_to_event_queue(new QueueProcessingEvent(packet->flow->get_current_time(), this, packet->flow->qid));
            this->busy = true;

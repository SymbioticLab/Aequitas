...
class Flow {
    public:
        ...
        virtual void send_ack(uint32_t seq, std::vector<uint32_t> sack_list, double pkt_start_ts);
        virtual void receive_ack(uint32_t ack, std::vector<uint32_t> sack_list, double pkt_start_ts, uint32_t priority);
